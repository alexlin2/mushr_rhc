...
class MXPI:
    ...
    def reset(self):
        ...
        self.desired_speed = self.params.get_float("trajgen/desired_speed",
                                              default=1.0)
        ...
        self.savgol_window = self.params.get_int("trajgen/mxpi/savgol/window", default=7)
        self.savgol_poly = self.params.get_int("trajgen/mxpi/savgol/poly", default=6)
        self.ctrl_k = self.params.get_float("trajgen/ctrl_k", default=10.0)
    ...
    def generate_control(self, controls, costs):
        ...
        if not self.fixed_vel:
            vels = torch.sum(-1 * (torch.abs(controls[:,:,0]) - self.desired_speed), dim=1) * 0.1
            vels += torch.sum(-1 * (controls[:,:,0] - self.desired_speed), dim=1) * 0.1
            costs.add_(vels)
        ...
        if self.use_savgol:
            v_noise_sum = torch.from_numpy(savgol_filter(v_noise_sum.numpy(), 
                self.savgol_window, self.savgol_poly))
            delta_noise_sum = torch.from_numpy(savgol_filter(delta_noise_sum.numpy(), 
                self.savgol_window, self.savgol_poly))
...
#        self.canonical_ctrl[:,0].clamp_(-self.desired_speed, self.desired_speed)
    ...
    def warmstart(self, control):
        assert control.size() == self.canonical_ctrl.size()
        self.canonical_ctrl[:] = control
        self.warmstarted = True
